...
def compute_intensity_vectorized(
    X_bottom: np.ndarray,
    Y_bottom: np.ndarray,
    X_interface: np.ndarray,
    Y_interface: np.ndarray,
    n_water: float,
    n_glass: float,
    water_depth: float,
    glass_thickness: float,
    water_absorption_coefficient: float,
    glass_absorption_coefficient: float,
    glass_size_mm: float,
    boundaries: Callable[[np.ndarray, np.ndarray, float], np.ndarray],
    absorption: bool
) -> np.ndarray:
    
    # Flatten bottom grid so we create all bottom×interface pairs correctly
    Xb = X_bottom.ravel()   # shape (Nb*Nb,)
    Yb = Y_bottom.ravel()   # shape (Nb*Nb,)

    # Broadcast to (Nb2, Ni, Ni)
    dx_water = X_interface[None, :, :] - Xb[:, None, None]
    dy_water = Y_interface[None, :, :] - Yb[:, None, None]
    ...
    sin_theta_g = (n_water / n_glass) * np.sin(theta_w)
    sin_theta_g = np.clip(sin_theta_g, -1.0, 1.0)
    theta_g = np.arcsin(sin_theta_g)
    ...
    x_top = X_interface[None, :, :] + dx_glass
    y_top = Y_interface[None, :, :] + dy_glass
    ...
    intensity_sum = np.nansum(intensity, axis=(1, 2))  # shape (Nb*Nb,)
    return intensity_sum.reshape(X_bottom.shape)

def run_sim_vectorized(
    glass_size_mm = 50.0,
    bottom_size_mm = 100.0,
    num_points_interface = 31,
    num_points_bottom = 31,
    glass_thickness = 1.6,
    water_depth = 5.0,
    n_glass = 1.45,
    n_water = 1.33,
    water_absorption_coefficient = 0.001,
    glass_absorption_coefficient = 1e-8,
    ...
    return compute_intensity_vectorized(
        X_bottom, Y_bottom, X_interface, Y_interface,
        n_water, n_glass,
        water_depth, glass_thickness,
        water_absorption_coefficient, glass_absorption_coefficient,
        glass_size_mm, boundaries, absorption
    )
