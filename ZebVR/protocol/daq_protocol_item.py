...
class DAQ_ProtocolItemWidget(ProtocolItemWidget):
    ...
    def declare_components(self) -> None:
        ...
        self.daq_board_type_cb = LabeledComboBox()
        self.daq_board_type_cb.setText('DAQ Board Type')
        for board_type in self.board_types:
            self.daq_board_type_cb.addItem(str(board_type))
        self.daq_board_type_cb.currentIndexChanged.connect(self.on_board_type_change)
        self.current_board_type = self.board_types[self.daq_board_type_cb.currentIndex()]
    ...
    def layout_components(self) -> None:
        ...
        self.main_layout.addWidget(self.daq_board_type_cb)
        self.main_layout.addWidget(self.daq_board_id_cb)

    def get_state(self) -> Dict:

        state = super().get_state()
        state['daq_board_type'] = self.current_board_type
        state['daq_board_id'] = self.current_board.id
        return state
    ...
    def set_state(self, state: Dict) -> None:
        ...
        set_from_dict(
            dictionary = state,
            key = 'daq_board_type',
            setter = self.daq_board_type_cb.setCurrentText,
            default = self.daq_board_type,
            cast = str
        )
    ...
    def from_protocol_item(self, protocol_item: ProtocolItem) -> None:
        ...
        if isinstance(protocol_item, DAQ_ProtocolItem):
            self.daq_board_type_cb.setCurrentText(str(protocol_item.daq_board_type))
