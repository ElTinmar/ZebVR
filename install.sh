...
if "$CONDA_EXE" env list | grep -q "ZebVR"; then
    ...
    sudo -u "$REAL_USER" CONDA_DEFAULT_CHANNELS="[]" "$CONDA_EXE" env update -f ZebVR.yml --prune
else
    ...
    sudo -u "$REAL_USER" CONDA_DEFAULT_CHANNELS="[]" "$CONDA_EXE" env create -f ZebVR.yml --yes
fi
